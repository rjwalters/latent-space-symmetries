...
A natural question arises: do neural networks trained on data with implicit symmetries develop \emph{internal} representations that respect those symmetries, even without explicit architectural constraints? Recent work on Group Crosscoders \cite{bhaskar2024group} provides a positive answer for vision: a convolutional network trained on natural images develops approximate dihedral group ($D_{32}$) equivariance in its early vision layers, with curve detectors exhibiting full rotational equivariance and line detectors showing 180$^\circ$ invariance.

We ask whether analogous structure exists in language. Specifically: do language model representations of permuted entities or reordered facts carry the algebraic structure of the symmetric group $S_n$? If ``Alice gave Bob the book'' and ``Bob gave Alice the book'' produce activations related by a linear operator $W_\pi$, do these operators compose as group elements---does $W_{\pi_1} W_{\pi_2} \approx W_{\pi_1 \circ \pi_2}$?

This question has practical significance beyond mechanistic interpretability. Modern approaches to synthetic data generation, such as the Simula framework \cite{davidson2026simula}, decompose data domains into independent factors sampled from hierarchical taxonomies. This product-space decomposition implicitly assumes that reordering or recombining content is semantically neutral---that the ``meaning'' of a fact set is invariant under permutation of its elements. If language models have learned that ordering encodes information---salience, narrative priority, discourse structure---then synthetic data approaches that scramble ordering may destroy signal the model has learned to use.

Our contributions are:
\begin{enumerate}[itemsep=2pt]
    \item A comprehensive empirical demonstration that language model representations are permutation-\emph{sensitive} but not permutation-\emph{equivariant}: linear operators fit well individually but fail all group composition tests ($S_3$ relation errors $\approx 1.0$, indistinguishable from random baselines).
    ...
    \item A conceptual framework distinguishing \emph{functional equivariance} (correct outputs under transformation) from \emph{structural equivariance} (group representations in latent space), explaining why the former is achievable without the latter under local optimization.
\end{enumerate}
...
\paragraph{Group structure in vision models.} Bhaskar et al.\ \cite{bhaskar2024group} introduced Group Crosscoders, training sparse autoencoders on concatenated activations from transformed inputs to discover symmetry structure. Applied to InceptionV1, they found approximate $D_{32}$ equivariance in early vision features. This positive result establishes that group structure \emph{can} emerge in neural networks trained on data with physical symmetries.
...
We study Qwen2.5-0.5B (24 layers, $d_\text{model} = 896$) and Qwen2.5-1.5B (28 layers, $d_\text{model} = 1536$), both base (non-instruct) models \cite{qwen2025}. Activations are captured using TransformerLens \cite{nanda2022transformerlens} at the residual stream (post-block) of every layer, at the last token position.
...
\paragraph{Positive control.} We construct synthetic activations with exact $S_3$ group structure: random base vectors $x \in \mathbb{R}^d$ mapped by exact representation matrices $W_\pi$ (the combined trivial + sign + standard irrep, embedded in $d = 10$ dimensions via random orthogonal rotation) plus calibrated Gaussian noise. This validates that our pipeline detects group structure when present.

\paragraph{Random baseline.} We fit operators on \emph{shuffled} prompt pairs---base activations from one prompt paired with ``permuted'' activations from an unrelated prompt. This establishes the chance-level floor for both fit quality and group relation errors.
...
\paragraph{SAE feature-space analysis.} Sparse autoencoders (8$\times$ expansion, 5M training tokens) trained on layers 0 and 21. Activations encoded through the SAE, operators fitted and group relations tested in the sparse feature basis.
...
\caption{Layer-by-layer analysis for $S_3$ entity permutations. (a)~Operator fit quality (test-set relative error) follows a U-shaped profile: best at early and late layers. Both model scales show the same pattern. (b)~Group relation errors remain $\approx 1.0$ at every layer regardless of fit quality, indicating no algebraic structure. The ``no structure'' baseline (gray dotted) at 1.0 represents the error expected from unrelated operators.}
\label{fig:layer_profile}
\end{figure}

\begin{table}[t]
\centering
\caption{Operator fit quality and group relation errors for $S_3$ across selected layers. Test error measures how well $W_\pi$ predicts held-out permuted activations. Group relation errors measure deviation from the $S_3$ presentation. An error of $\approx 1.0$ indicates no better than random composition.}
\label{tab:alignment}
\small
\begin{tabular}{@{}lcccccc@{}}
\toprule
& \multicolumn{3}{c}{Qwen2.5-0.5B} & \multicolumn{3}{c}{Qwen2.5-1.5B} \\
\cmidrule(lr){2-4} \cmidrule(lr){5-7}
Layer & Test Err & $s^2\!=\!e$ & $r^3\!=\!e$ & Test Err & $s^2\!=\!e$ & $r^3\!=\!e$ \\
\midrule
0  & 0.078 & 1.023 & 1.094 & 0.123 & 0.987 & 1.006 \\
5  & 0.249 & 1.040 & 1.040 & 0.224 & 1.012 & 1.018 \\
10 & 0.408 & 1.021 & 1.020 & 0.323 & 1.012 & 1.020 \\
16 & 0.429 & 1.031 & 1.031 & 0.439 & 1.011 & 1.009 \\
21 & 0.258 & 1.015 & 1.024 & 0.521 & 1.013 & 1.011 \\
\midrule
\multicolumn{7}{l}{\textit{Positive control ($\sigma = 0.1$, $d = 10$)}} \\
--- & 0.101 & 0.045 & 0.061 & & & \\
\midrule
\multicolumn{7}{l}{\textit{Random baseline (shuffled pairs, layer 0)}} \\
--- & 1.675 & 36.9 & 158.1 & & & \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Group relations: the null result.} Despite good individual operator fits, all $S_3$ presentation relations fail (Table~\ref{tab:alignment}). Relation errors are $\approx 1.0$ at every layer, for both model scales. The positive control demonstrates that the pipeline detects group structure when present: at noise levels yielding comparable test error ($\sim$0.1), relation errors are 0.04--0.06---twenty times lower than the real model's $\sim$1.0. The random baseline confirms that the permutation signal is real (operators fit 21$\times$ better than shuffled pairs) but the algebraic structure is not (relation errors are comparable to random).

\paragraph{The pattern is robust.} We tested every natural place group structure might hide:
\begin{itemize}[itemsep=1pt]
    \item \textbf{PCA subspace} (3--200D): Best result at layer 0, dim 3: $s^2 = e$ error 0.09, but $r^3 = e$ still fails at 0.73.
    \item \textbf{Irrep search}: No sign representation or standard 2D irrep detected at any layer.
    \item \textbf{SAE feature space}: Group relations \emph{worse} than raw activation space (errors 1--34$\times$).
    \item \textbf{Eigenvalue analysis}: Transposition operators have eigenvalues clustered near $+1$ with none near $-1$ and $\det \approx 0$---they are rank-deficient projections, not reflections.
    \item \textbf{Scale}: The 1.5B model shows identical patterns across all 28 layers.
\end{itemize}
...
\paragraph{Random baseline.} At layer 0, shuffled-pair operators fit with test error 1.68 (vs.\ 0.08 for real pairs---a 21$\times$ ratio). But group relation errors from shuffled pairs are comparable to real pairs: $s^2 = e$ error 36.9 (shuffled) vs.\ 1.02 (real). The real model's relation errors, while lower than random, are still $\sim$1.0---firmly in the ``no structure'' regime. The signal that makes real operators fit well does not carry algebraic coherence.
...
\begin{table}[t]
\centering
\caption{Impact of fact reordering on model representations. ``Within'' = same facts in different order. ``Between'' = entirely different facts. Ratio $\ll 1$ would indicate permutation invariance; observed ratios of 0.35--0.60 show ordering substantially affects representations.}
\label{tab:facts}
\small
\begin{tabular}{@{}lcccccc@{}}
\toprule
& \multicolumn{3}{c}{2-fact ($S_2$)} & \multicolumn{3}{c}{3-fact ($S_3$)} \\
\cmidrule(lr){2-4} \cmidrule(lr){5-7}
Layer & Within & Between & Ratio & Within & Between & Ratio \\
\midrule
0  & 0.112 & 0.186 & 0.60 & 0.094 & 0.200 & 0.47 \\
5  & 0.166 & 0.476 & 0.35 & 0.176 & 0.501 & 0.35 \\
10 & 0.289 & 0.567 & 0.51 & 0.307 & 0.606 & 0.51 \\
21 & 0.200 & 0.516 & 0.39 & 0.203 & 0.545 & 0.37 \\
23 & 0.265 & 0.685 & 0.39 & 0.277 & 0.745 & 0.37 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Behavioral impact.} Next-token predictions change meaningfully with fact order: top-1 agreement is only 67\% for 2-fact and 71\% for 3-fact sequences (Table~\ref{tab:behavioral}). Mean KL divergence is 0.35 ($S_2$) and 0.28 ($S_3$). The model produces \emph{different continuations} for the same facts in different orders---confirming that ordering carries semantic weight.

\begin{table}[t]
\centering
\caption{Behavioral impact of fact reordering. Top-1 agreement below 100\% indicates the model's most likely next token changes when facts are reordered.}
\label{tab:behavioral}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
& Mean KL & Top-1 Agreement & Even KL / Odd KL \\
\midrule
2-fact ($S_2$) & 0.355 & 66.7\% & --- \\
3-fact ($S_3$) & 0.285 & 70.7\% & 0.366 / 0.231 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Case study.} Consider three facts of varying urgency: (F1)~``The room is on fire.'' (F2)~``I am sitting at my desk researching internal representations in LLMs.'' (F3)~``It is Wednesday.'' The KL divergence matrix across all 6 orderings (Figure~\ref{fig:kl_heatmap}) reveals: orderings ending with the same fact produce similar predictions (KL $= 0.026$ between orderings F-W-R and W-F-R, both ending with the research fact), while orderings ending with different facts diverge substantially (KL $= 0.35$ between F-R-W and R-W-F). The model attends most to the \emph{last-mentioned} fact---a recency effect, not a symmetry.

\begin{figure}[t]
\centering
\includegraphics[width=0.65\textwidth]{figures/fig3_kl_heatmap.pdf}
\caption{KL divergence between next-token distributions for all 6 orderings of three facts: ``The room is on fire'' (F), ``I am researching LLM representations'' (R), ``It is Wednesday'' (W). Orderings sharing the same final fact cluster together (block structure along anti-diagonal), revealing a recency effect. Tick labels are colored by final fact.}
...
\paragraph{(B) The map.} Transformers cannot learn group structure even if it exists. But Group Crosscoders \cite{bhaskar2024group} refute the strong version: vision models \emph{do} learn approximate group structure when the physical world has it (rotation of edges). The architecture is capable; the question is whether the data demands it.

\paragraph{(C) The reward.} Next-token prediction does not incentivize algebraic coherence. Composing permutation operators is never required for correct prediction. A per-permutation linear map is sufficient and has strictly more capacity than a group-constrained representation ($n!$ independent matrices vs.\ matrices satisfying $O(n!^2)$ algebraic constraints).

The likely answer combines (A) and (C): the symmetry is not present in natural language, \emph{and} the training objective would not reward learning it even in domains where it exists. Group structure in representations is not free---it requires either architectural inductive bias (equivariant layers), data with genuine symmetry (physical rotations), or an objective that rewards compositional generalization over group orbits.

\subsection{Functional vs.\ Structural Equivariance}

Our models achieve functional equivariance: swapping Alice and Bob reliably produces a swapped completion. But they do not achieve structural equivariance: the internal operators implementing this swap do not compose as a group. The model has learned a \emph{lookup table}, not an \emph{algebra}.

This distinction has a parallel in human cognition. Humans develop functional world models from temporally ordered experience, optimized by reward signals (dopamine, social feedback) that select for prediction accuracy rather than structural fidelity. The result is mostly-consistent representations with tolerance for cognitive dissonance---functional without being algebraically coherent. In physics, discovering that functional regularities arise from structural symmetries required stepping \emph{outside} the local optimization landscape---a contribution of theoretical reasoning, not of the optimization process itself.
...
\paragraph{Level 1: Fact ordering is not a nuisance variable.} Reordering the same facts changes model predictions 30\% of the time. If synthetic training data presents facts in arbitrary order, it generates examples with different implicit salience signals. This is directly measurable and directly actionable.

\paragraph{Level 2: Product-space decomposition has blind spots.} Frameworks like Simula \cite{davidson2026simula} decompose data domains as $\mathcal{T}_0 \times \mathcal{T}_1 \times \cdots \times \mathcal{T}_K$ and sample factors independently. Our results show that trained models have learned structure that cannot be decomposed as a product of independent factors. Ordering, salience, and discourse structure create correlations between factors that product-space sampling misses.
...
We tested only small symmetric groups ($S_2$, $S_3$); larger groups or continuous symmetries may behave differently. Our experiments use two models from the same family (Qwen2.5); architecture-specific effects are possible. The fact-level permutation experiments use relatively simple multi-sentence sequences; more complex discourse structures may show different patterns. We did not test whether training with permutation-augmented data would \emph{induce} group structure, nor did we directly compare model performance when fine-tuned on ordered vs.\ shuffled synthetic data. Our positive control uses low-dimensional spaces where the system is well-conditioned; the analogy to high-dimensional neural activations is approximate.

%----------------------------------------------------------------------
\section{Future Work}
\label{sec:future}

Several directions follow naturally: (1)~Train models on permutation-augmented data and measure downstream impact on the presence of group structure and on task performance. (2)~Test whether explicitly equivariant architectures develop group representations for linguistic permutations. (3)~Extend to other symmetries in language: negation ($\mathbb{Z}_2$), tense shifts, semantic role reversals. (4)~Investigate whether biological visual systems (via fMRI population codes) show rotational group structure, paralleling the Group Crosscoders finding in artificial vision models. (5)~Examine weight matrices directly for emergent group structure, complementing our activation-level analysis.
...
\bibitem{bhaskar2024group}
D.~Bhaskar, J.~Benton, and S.~Racaniere, ``Group crosscoders for mechanistic analysis of symmetry,'' \textit{arXiv preprint arXiv:2410.24184}, 2024.
...
D.~Xu, S.~Cheng, and Z.~Wang, ``Permutation equivariance of transformers and its applications,'' in \textit{Proc.\ CVPR}, 2024.
